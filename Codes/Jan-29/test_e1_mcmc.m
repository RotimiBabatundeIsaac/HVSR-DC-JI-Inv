...
n_h = numel(prior.idx.h);
n_vs = numel(prior.idx.vs);
fprintf('Model: %d layers (%d thicknesses + %d velocities = %d parameters)\n', n_vs, n_h, n_vs, prior.n_params);
fprintf('\nPrior bounds:\n');
fprintf('  Thicknesses (km):\n');
for i = 1:n_h
    fprintf('    %s: [%.4f, %.4f]\n', prior.param_names{i}, ...
        prior.bounds.lower(i), prior.bounds.upper(i));
end
fprintf('  Velocities (km/s):\n');
for i = 1:n_vs
    idx = prior.idx.vs(i);
    fprintf('    %s: [%.4f, %.4f]\n', prior.param_names{idx}, ...
        prior.bounds.lower(idx), prior.bounds.upper(idx));
end
fprintf('\n');
...
n_layers_test = numel(prior.idx.vs);
h_test = 0.5 * (prior.bounds.lower(prior.idx.h) + prior.bounds.upper(prior.idx.h));
vs_test = 0.5 * (prior.bounds.lower(prior.idx.vs) + prior.bounds.upper(prior.idx.vs));
model_test = zeros(n_layers_test, 4);
for i = 1:n_layers_test
    if i <= numel(h_test)
        model_test(i,1) = h_test(i);
    else
        model_test(i,1) = 0;
    end
...
model_ext(end,1) = 5;
for i = 1:8
    model_ext(end+1,:) = [12, 8.0, 4.5, 3.3];
end
model_ext(end,1) = 0;
...
fprintf('Thickness estimates (km):\n');
for i = 1:numel(prior.idx.h)
    idx = prior.idx.h(i);
    fprintf('  %s: %.4f +/- %.4f [%.4f, %.4f]\n', ...
        prior.param_names{idx}, results.theta_mean(idx), results.theta_std(idx), ...
        results.theta_q05(idx), results.theta_q95(idx));
end

fprintf('\nVelocity estimates (km/s):\n');
for i = 1:numel(prior.idx.vs)
    idx = prior.idx.vs(i);
    fprintf('  %s: %.4f +/- %.4f [%.4f, %.4f]\n', ...
        prior.param_names{idx}, results.theta_mean(idx), results.theta_std(idx), ...
        results.theta_q05(idx), results.theta_q95(idx));
...
fprintf('\nTotal depth: %.2f +/- %.2f km\n', ...
    sum(results.theta_mean(prior.idx.h)), ...
    sqrt(sum(results.theta_std(prior.idx.h).^2)));

fprintf('\nMAP estimate (max log-likelihood = %.2f):\n', results.map_logL);
fprintf('  h (km): ');
fprintf('%.3f ', results.theta_map(prior.idx.h));
fprintf('\n');
fprintf('  Vs (km/s): ');
fprintf('%.3f ', results.theta_map(prior.idx.vs));
fprintf('\n');
...
h_map = theta_map(prior.idx.h);
vs_map = theta_map(prior.idx.vs);
...
hold on;
for i = 1:n_plot
    h_i = results.all_theta(plot_idx(i), prior.idx.h);
    vs_i = results.all_theta(plot_idx(i), prior.idx.vs);
    plot_layer_profile(h_i(:), vs_i(:), 70, '-', 0.5, [0.75 0.75 0.75]);
end

plot_layer_profile(h_map(:), vs_map(:), 70, '-', 2.5, [1 0 0]);
...
Ns = results.n_total;
h_samps = results.all_theta(:, prior.idx.h);
vs_samps = results.all_theta(:, prior.idx.vs);
...
fig_dir = '/Users/birotimi/Downloads/HVSR-Joint-Inversion/Codes/Mod5_HBI/figures_new_test_10laymcmc';
...
save(results_file, 'results', 'prior', 'data', 'pred_map', 'pdf_zvs', 'z_grid', 'vs_vec', 'theta_map', 'h_map', 'vs_map');
