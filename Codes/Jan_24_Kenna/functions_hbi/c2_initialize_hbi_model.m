...
if need_reset
    new_path = [cps_bin, pathsep, hvf_bin, pathsep, '/usr/bin:', '/bin'];
...
for i = 1:n_params
    fprintf('  %s = %.4f %s\n', prior.param_names{i}, theta(i), prior.param_units{i});
end
fprintf('[c2] Initialized sigma_e: [%.4f, %.4f, %.4f, %.4f]\n', ...
    model.sigma_e(1), model.sigma_e(2), model.sigma_e(3), model.sigma_e(4));
